...
@pytest.fixture
def expected_s3_file_keys_pre_update():
    """Return the expected s3 file keys pre update."""
    return [
...
@pytest.fixture
def expected_rds_documents_pre_update():
    """Get the expected documents from the rds database pre the update bulk import."""
    return [
        {
            "country_code": "GNQ",
            "country_name": "Equatorial Guinea",
            "description": "This decree approves the National Strategy for Sustainable "
            "Development. The strategy contains the final act of the "
            "Third National Economic Conference; the PNDES 2020; the "
            "Equatorial Guinea Agenda 2035; the Strategic Plan for "
            "Sustainable Development 2020-2025; and the Supporting "
            "Programme to Economic Diversification (PRODECO).",
            "document_id": 4,
            "import_id": "CCLW.executive.10992.6271",
            "name": "Information on Decree 69/2021 approving the National Strategy for "
            'Sustainable Development "Agenda Guinea Ecuatorial 2035"',
            "postfix": "",
            "publication_ts": "2021-01-01T00:00:00",
            "slug": "equatorial-guinea_2021_information-on-decree-69-2021-approving-the-national-strategy-for-sustainable"
            "-development-agenda-guinea-ecuatorial-2035_10992_6271",
        },
        {
            "document_id": 5,
        ...
        },
        {
            ...
            "name": "National REDD+ Investment Plan (PNI-REDD+)",
            "postfix": "",
...
def test_original_bulk_import_state_s3(expected_s3_file_keys_pre_update):
    """
    ...
    """
    s3_files = [
        path_.key for path_ in S3Path(f"s3://{PIPELINE_BUCKET}/").glob("*/*.json")
    ]

    assert s3_files == expected_s3_file_keys_pre_update


def test_original_bulk_import_state_rds(expected_rds_documents_pre_update):
    """
    Assert that the initial state of the test infrastructure is correct prior to the bulk import with updates.
    ...
    assert json.loads(rds_documents.text) == expected_rds_documents_pre_update
