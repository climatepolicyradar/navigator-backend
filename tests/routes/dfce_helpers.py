...
from db_client.models.dfce import Collection, Family, CollectionOrganisation, FamilyOrganisation, CollectionFamily, FamilyDocument, FamilyEvent, Slug, FamilyMetadata
...
def add_collections(db: Session, collections):
    ...
    db.flush()
    for c in collections:
        db.add(CollectionOrganisation(
           collection_import_id=c["import_id"],
           organisation_id=1
        ))
    db.commit()
...
def add_families(db: Session, families):
    for f in families:
        db.add(Family(
