...
def create_label(
    session: Session, label_id: str, title: str, type_: str = "entity_type"
) -> DBLabel:
    label = DBLabel(id=label_id, title=title, type=type_)
    session.add(label)
    session.commit()
    return label
...
def link_document_label(
    session: Session, doc_id: str, label_id: str, relationship_type: str = "entity_type"
) -> DBDocumentLabelLink:
    link = DBDocumentLabelLink(
        document_id=doc_id, label_id=label_id, relationship_type=relationship_type
    )
    session.add(link)
    session.commit()
    return link
...
def test_get_all_documents_with_relationships(setup_documents: Session):
    ...
    assert doc1.labels[0].label.title == "Main"
    assert doc1.relationships[0].document.id == "doc2"
    assert doc1.relationships[0].document.items[0].url == "https://example.com/2"
...
def test_get_document_by_id_with_relationships(setup_documents: Session):
    ...
    assert doc1.id == "doc1"
    assert doc1.labels[0].label.id == "Main"
    assert doc1.items[0].url == "https://example.com/1"
    assert doc1.relationships[0].type == "member_of"
    assert doc1.relationships[0].document.id == "doc2"
