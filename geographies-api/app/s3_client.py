...
class S3Client:
    ...
    def upload_json(
        ...
        :param data: Dictionary containing the JSON data to upload
        :type data: t.Dict[str, t.Any]
        :param bucket: Name of the S3 bucket
        :type bucket: str
        :param key: S3 key (path) where the file should be uploaded
        :type key: str
        :return: S3Document if upload succeeds, False if it fails
        :rtype: t.Union[S3Document, bool]
        """
        try:
            logger.info(f"Converting JSON data for upload to s3://{bucket}/{key}")
            json_str = json.dumps(data)
            json_bytes = json_str.encode("utf-8")
            fileobj = BytesIO(json_bytes)

            logger.info(f"Uploading JSON data to s3://{bucket}/{key}")
            return self.upload_fileobj(
                fileobj=fileobj, bucket=bucket, key=key, content_type="application/json"
            )
