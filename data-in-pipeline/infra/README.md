...
1. Use the [`nav-stack-connect`](../backend/nav-stack-connect.sh) script to
   connect to the bastion

2. When the two terminals open, copy the following socat command into the
   terminal window with the ASCII text:

   ```bash
   # Where ${ENVIRONMENT} is the AWS environment
   # Look in AWS Aurora console to replace XXXXXXXXXXXX
