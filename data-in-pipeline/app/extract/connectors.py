...
from app.models import ExtractedEnvelope, ExtractedMetadata, IncomingConcept
...
class NavigatorDocument(BaseModel):
    ...
    They are nullable because a `FamilyDocument` does not need to have an attached `PhysicalDocument`.
    """
    languages: list[str] = []
...
class NavigatorFamily(BaseModel):
    ...
    category: str
    slug: str
    concepts: list[IncomingConcept] = Field(default_factory=list)
