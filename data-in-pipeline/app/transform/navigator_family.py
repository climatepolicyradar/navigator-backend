...
def transform_navigator_family_with_single_matching_document(
    ...
    if (
        ...
        return Success(
            [transform_navigator_family_document(input, input.data.documents[0])]
        )
    else:
        return Failure(
            CouldNotTransform(
...
def transform_navigator_family_document(
    ...
    labels = [DocumentLabelRelationship(type="family", label=family_label)]
