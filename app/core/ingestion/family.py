...
def new_family_from_row(
    ...
    """
    Create a Family + other entities from the row only.
    ...
    """
    family = _create_family(db, row, org_id, result)

    _create_family_document(db, row, family, existing_document, result)

    return family


def migrate_family_from_row(
    db: Session,
    row: DocumentIngestRow,
    existing_document: Document,
    org_id: int,
...
def _create_family(
    ...
    if not _validate_family_name(db, row.family_name, row.cpr_family_id):
        raise ValueError(
            ...
            "that is different to an existing family title for family id "
            f"{row.cpr_family_id}"
        )
