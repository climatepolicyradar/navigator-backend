...
Revision ID: 0013
Revises: 0012
Create Date: 2023-01-27 11:57:58.051145
...
def upgrade():
    ...
    sa.ForeignKeyConstraint(['document_type'], ['document_type.document_type'], name=op.f('fk_family_document__document_type__document_type')),
    ...
    op.drop_constraint('fk_document__type_id__document_type', 'document', type_='foreignkey')
    op.create_foreign_key(op.f('fk_document__type_id__document_type'), 'document', 'document_type', ['type_id'], ['document_type'])
    op.add_column('document_type', sa.Column('document_type', sa.Text(), nullable=False))
    op.drop_constraint('uq_document_type__name', 'document_type', type_='unique')
    op.drop_column('document_type', 'id')
    op.drop_column('document_type', 'name')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('document_type', sa.Column('name', sa.TEXT(), autoincrement=False, nullable=False))
    op.add_column('document_type', sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('document_type_id_seq'::regclass)"), autoincrement=True, nullable=False))
    op.create_unique_constraint('uq_document_type__name', 'document_type', ['name'])
    op.drop_column('document_type', 'document_type')
    op.drop_constraint(op.f('fk_document__type_id__document_type'), 'document', type_='foreignkey')
    op.create_foreign_key('fk_document__type_id__document_type', 'document', 'document_type', ['type_id'], ['id'])
    op.drop_table('slug')
